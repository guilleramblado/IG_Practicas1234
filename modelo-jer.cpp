// Nombre: Guillermo, Apellidos: Ramblado Carrasco, Titulación: DGIIM
// email: guilleramblado@correo.ugr.es DNI:29628168M
...
Robot::Robot(const float angulo_rotacion, const float longitud_cuello, const float rotacion_cabeza){
    ...
    Textura *textura_brazos = new Textura("text-metalica.jpg");
    Material *material_brazos = new Material(textura_brazos,0.2,0.0,0.8,1.0);
    torso->agregar(material_brazos);
    ...
    Material *material_pecho = new Material(0.2,0.5,0.3,10.0);
    torso->agregar(material_pecho);
    torso->agregar(new Pecho());
    torso->agregar(translate(vec3{0,1.5,0})); //ajustamos cabeza+cuello al PECHO
    //metemos el material a usar sobre la cabeza-cuello
    //material difuso, usando las tablas de coordenadas de textura
    Textura *text_cabezacuello = new Textura("rugoso.jpg");
    Material *material_cabezacuello = new Material(text_cabezacuello,0.2,0.8,0.0,1.0);
    torso->agregar(material_cabezacuello);
    torso->agregar(cabezacuello);

    //pegamos el torso con las piernas
    agregar(torso);
    agregar(translate(vec3{1.05,-2.3,0.25}));
    //metemos el material de la pierna
    //usamos un material pseudo-especular (brillo concentrado en ciertas areas)
    Textura *text_pierna = new TexturaXY("text-metalica.jpg");
    Material *material_pierna = new Material(text_pierna,0.2,0.0,0.8,1.0);
    agregar(material_pierna);
    agregar(piernas);
